...
public class VoxelizedMesh : MonoBehaviour
{
    ...
    [SerializeField] int _distributeHeatPasses = 1;
    [SerializeField, Range(0, 20)] int _boneIndex;
    ...
    void VoxelizeMeshWithGPU()
    {
        ...
        _voxelizeComputeShader.SetInt("_TriangleCount", mesh.triangles.Length);
        ...
        var clearBoneHeatKernel = _voxelizeComputeShader.FindKernel("ClearBoneHeat");
        _voxelizeComputeShader.SetBuffer(clearBoneHeatKernel, Voxels, _voxelsBuffer);
        _voxelizeComputeShader.GetKernelThreadGroupSizes(clearBoneHeatKernel, out xGroupSize, out yGroupSize,
            out zGroupSize);
        _voxelizeComputeShader.Dispatch(clearBoneHeatKernel,
            Mathf.CeilToInt(xGridSize / (float) xGroupSize),
            Mathf.CeilToInt(yGridSize / (float) yGroupSize),
            Mathf.CeilToInt(zGridSize / (float) zGroupSize));

        var setStartBoneKernel = _voxelizeComputeShader.FindKernel("SetStartBone");
        _voxelizeComputeShader.SetVector("_StartBonePosition", _skinnedMeshRenderer.bones[_boneIndex].position);
        _voxelizeComputeShader.SetBuffer(setStartBoneKernel, Voxels, _voxelsBuffer);
        _voxelizeComputeShader.GetKernelThreadGroupSizes(setStartBoneKernel, out xGroupSize, out yGroupSize,
            out zGroupSize);
        _voxelizeComputeShader.Dispatch(setStartBoneKernel,
            Mathf.CeilToInt(xGridSize / (float) xGroupSize),
            Mathf.CeilToInt(yGridSize / (float) yGroupSize),
            Mathf.CeilToInt(zGridSize / (float) zGroupSize));

        var distributeHeatKernel = _voxelizeComputeShader.FindKernel("DistributeBoneHeat");
        _voxelizeComputeShader.SetBuffer(distributeHeatKernel, Voxels, _voxelsBuffer);
        _voxelizeComputeShader.GetKernelThreadGroupSizes(distributeHeatKernel, out xGroupSize, out yGroupSize,
            out zGroupSize);

        for (int i = 0; i < _distributeHeatPasses; ++i)
        {
            _voxelizeComputeShader.Dispatch(distributeHeatKernel,
                Mathf.CeilToInt(xGridSize / (float) xGroupSize),
                Mathf.CeilToInt(yGridSize / (float) yGroupSize),
                Mathf.CeilToInt(zGridSize / (float) zGroupSize));
        }
